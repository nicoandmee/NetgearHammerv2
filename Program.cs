    ...
    class Program {
        ...
        static async Task Main (string[] args) {
            ...
            for (int i = 0; i < Convert.ToInt32 (attemptsCount); i++) {
                ...
                string result = await checkSerial (tempSerial, page);

                if (result.Contains ("None")) {
                    invalidCounter++;
                } else {
                    Console.WriteLine ($"[+] Valid Serial: {tempSerial}");
                    string productData = await extractData (tempSerial, page);
            ...
            Console.WriteLine ("We found " + invalidCounter + " invalid serials out of the total attempt count of " + attemptsCount);
        ...
        static async Task<string> extractData (string serialNumber, Page page) {
            ...
            dynamic productData = await page.EvaluateExpressionAsync<dynamic> ("document.querySelector('#formPop').innerText");
            return productData;
        }
        ...
        static async Task<string> checkSerial (string serialNumber, Page page) {
            ...
            try {
                ...
                dynamic result = await page.EvaluateExpressionAsync<dynamic> ("((document.querySelector('#MainContent_errorStatus,.activeServerError') || {}).innerText || '').trim();");
